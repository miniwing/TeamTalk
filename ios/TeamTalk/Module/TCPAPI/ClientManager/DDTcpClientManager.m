...
- (void)connect:(NSString *)aIP port:(NSInteger)aPort status:(NSInteger)aStatus {
   ...
   NSInputStream                 *stInputStream                            = nil;
   NSOutputStream                *stOutputStream                           = nil;
   ...
   [NSStream getStreamsToHostNamed:aIP port:aPort inputStream:&stInputStream outputStream:&stOutputStream];
   
   _inStream   = stInputStream;
   _outStream  = stOutputStream;
...
//            [[DDAPISchedule instance] receiveServerData:_receiveBuffer];
            ...
            //                debugLog(@"receive a packet serviceId=%d, commandId=%d", tcpHeader.serviceId, tcpHeader.commandId);
            ...
            //                debugLog(@"***********收到服务端sid:%i cid:%i",tcpHeader.serviceId,tcpHeader.commandId);

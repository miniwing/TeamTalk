...
#if __TEAMTALK_FRIEND__
         self.user.isFriend   = [(MTTUserEntity*)response[0] isFriend];
#else
         self.user.isFriend   = YES;
#endif /* __TEAMTALK_FRIEND__ */
...
- (UIView *)footView
{
   ...
   if(![self.user.objID isEqualToString:TheRuntime.user.objID]){
      ...
      if (self.user.isFriend == 1) {
         ...
         [_chatBtn setTitle:@"加好友" forState:UIControlStateNormal];
         [_chatBtn addTarget:self action:@selector(startAddFriend) forControlEvents:UIControlEventTouchUpInside];
      }
#else
      [_chatBtn setTitle:@"发送消息" forState:UIControlStateNormal];
      [_chatBtn addTarget:self action:@selector(startChat) forControlEvents:UIControlEventTouchUpInside];
...
- (void)startChat {
   ...
   MTTSessionEntity              *stSession                                = nil;
   
   __TRY;

   stSession   = [[MTTSessionEntity alloc] initWithSessionID:self.user.objID type:SessionTypeSessionTypeSingle];
   [[ChattingMainViewController shareInstance] showChattingContentForSession:stSession];

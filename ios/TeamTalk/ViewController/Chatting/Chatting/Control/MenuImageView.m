...
-(BOOL)canPerformAction:(SEL)action withSender:(id)sender
{
    ...
    else if (_showMenu & DDShowDelete && action == @selector(deleteOneMessage:))
    {
        return YES;
    }
    else if (_showMenu & DDShowRepost && action == @selector(repostOneMessage:))
    {
        return YES;
    }
    return NO;
...
- (void)showTheMenu
{
    ...
    UIMenuItem *deleteItem = [[UIMenuItem alloc] initWithTitle:@"删除" action:@selector(deleteOneMessage:)];
    UIMenuItem *repostItem = [[UIMenuItem alloc] initWithTitle:@"转发" action:@selector(repostOneMessage:)];
    
    UIMenuController *menu = [UIMenuController sharedMenuController];
    [menu setMenuItems:nil];
    [menu setMenuItems:[NSArray arrayWithObjects:copyItem,sendAgain,earphonePlayItem, speakerPlayItem,previewItem, deleteItem, repostItem, nil]];
    [menu setTargetRect:self.frame inView:self.superview];
    [menu setMenuVisible:YES animated:YES];
    debugLog(@"menuItems:%@",menu.menuItems);
}
...
- (void)deleteOneMessage:(id)sender
{
...
- (void)repostOneMessage:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(clickTheRepost:)])
    {
        [self.delegate clickTheRepost:self];
    }
}
@end

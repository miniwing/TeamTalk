...
@interface ChatUtilityViewController () <LocationViewControllerDelegate, TZImagePickerControllerDelegate>
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets{
    [[ChattingMainViewController shareInstance] dismissViewControllerAnimated:YES completion:nil];
    for (UIImage *orgImage in photos) {
        MTTPhotoEnity *photo = [MTTPhotoEnity new];
        NSString *keyName = [[MTTPhotosCache sharedPhotoCache] getKeyName];
        NSData *photoData = UIImageJPEGRepresentation(orgImage, 0.5);
        [[MTTPhotosCache sharedPhotoCache] storePhoto:photoData forKey:keyName toDisk:YES];
        photo.localPath=keyName;
        photo.image=orgImage;

...
- (void)sendLocationMessage:(NSString *)latitude longitude:(NSString *)longitude andAddress:(NSString *)address
{
   ...
   MTTMessageEntity *message = [MTTMessageEntity makeMessage:messageContent Module:self.module MsgType:DDMEssageTypeLocation];
   [self.tableView reloadData];
   [self scrollToBottomAnimated:YES];
   [[MTTDatabaseUtil instance] insertMessages:@[message] success:^{
      DDLog(@"消息插入DB成功");
   ...
   //    ModifyPassAPI *pushToken = [[ModifyPassAPI alloc] init];
   //    [pushToken requestWithObject:@[@"121212", @"111111"] Completion:^(id response, NSError *error) {
   //        debugLog(@"%@", response);
   //    }];
}
...
- (void)recordingFinishedWithFileName:(NSString *)filePath time:(NSTimeInterval)interval {
   ...
   if (length < 1 ) {
      ...
      DDLog(@"录音时间太短");
      LogDebug((@"-[ChattingMainViewController recordingFinishedWithFileName:time:] : 录音时间太短"));
   ...
   //    NSDictionary* messageContentDic = @{DD_VOICE_LOCAL_KEY:filePath, VOICE_LENGTH:[NSString stringWithFormat:@"%d", length]};
   //    NSString* messageContent = [messageContentDic jsonString];
   //
   //    MTTMessageEntity *message = [MTTMessageEntity makeMessage:messageContent Module:self.module MsgType:DDMessageTypeVoice];
   //    [self.tableView reloadData];
   //    [self scrollToBottomAnimated:YES];
   //    [[MTTDatabaseUtil instance] insertMessages:@[message] success:^{
   //        DDLog(@"消息插入DB成功");
   ...
   //        DDLog(@"消息插入DB失败");
   //    }];
   //    [[DDSendVoiceMessageAPI sharedVoiceCache] uploadVoice:messageContentDic[DD_VOICE_LOCAL_KEY] success:^(NSString *voiceURL) {
   //        [self scrollToBottomAnimated:YES];
   //        message.state=DDMessageSending;
   //        NSDictionary* tempMessageContent = [NSDictionary initWithJsonString:message.msgContent];
   //        NSMutableDictionary* mutalMessageContent = [[NSMutableDictionary alloc] initWithDictionary:tempMessageContent];
   //        [mutalMessageContent setValue:voiceURL forKey:DD_IMAGE_URL_KEY];
   //        NSString* messageContent = [mutalMessageContent jsonString];
   //        message.msgContent = messageContent;
   //        [self sendMessage:voiceURL messageEntity:message];
   //        [[MTTDatabaseUtil instance] updateMessageForMessage:message completion:^(BOOL result) {
   //        }];
   //
   //    } failure:^(id error) {
   ...
   [[DDMessageSendManager instance] sendVoiceMessage:muData
                                            filePath:filePath
                                        forSessionID:self.module.MTTSessionEntity.sessionID
                                             isGroup:isGroup
                                             Message:message
                                             Session:self.module.MTTSessionEntity
                                          completion:^(MTTMessageEntity *theMessage, NSError *error) {
      if (!error)
      {
         DDLog(@"发送语音消息成功");
...
- (void)deleteTheMessage:(MTTMessageEntity *)message
{
...
- (UITableViewCell*)p_voiceCell_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath message:(MTTMessageEntity*)message
{
   ...
   //        if (messageContent[DD_VOICE_LOCAL_KEY])
   //        {
   //            fileName = messageContent[DD_VOICE_LOCAL_KEY];
   //        }
   ...
   //        }
   //        else {
   //            [weakSelf downVoice:messageContent[DD_VOICE_URL_KEY] cell:weakCell displayType:0 ofMessage:message];
   //        }
   //    }];
   //
   ...
   //        }
   //        else {
   //            [weakSelf downVoice:messageContent[DD_VOICE_URL_KEY] cell:weakCell displayType:1 ofMessage:message];
   //        }
   //
   //    }];
   //
   ...
   //        }
   //        else {
   //            [weakSelf downVoice:messageContent[DD_VOICE_URL_KEY] cell:weakCell displayType:2 ofMessage:message];
   //        }
   //
   //    }];
...
- (void)downVoice:(NSString *)url cell:(DDChatVoiceCell *)cell displayType:(int)type ofMessage:(MTTMessageEntity *)message
{
   ...
   if ([fileManager fileExistsAtPath:filePath]) {
      if (type == 0) {
         [[PlayerManager sharedManager] playAudioWithFileName:filePath playerType:DDEarPhone delegate:self];
         [message.info setObject:@(1) forKey:DDVOICE_PLAYED];
         [cell showVoicePlayed];
      ...
      __weak DDChatVoiceCell* weakCell = cell;
      [self downloadFileWithOption:nil
                     withInferface:url
                         savedPath:filePath
                   downloadSuccess:^(NSURL *  fileURL) {
         [weakCell showSendSuccess];
         if (type == 0) {
            [[PlayerManager sharedManager] playAudioWithFileName:filePath playerType:DDEarPhone delegate:self];
            [message.info setObject:@(1) forKey:DDVOICE_PLAYED];
            [weakCell showVoicePlayed];
...

{
   ...
   downloadTask = [manager downloadTaskWithRequest:request
      ...
      return [documentsDirectoryURL URLByAppendingPathComponent:[NSString stringWithFormat:@"File-%@/%@", @"voice", [NSString stringWithFormat:@"%@.spx", [Tool getMd5_16Bit_String:requestURL]]]];
...
- (UITableViewCell*)p_locationCell_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath message:(MTTMessageEntity*)message
{
   static NSString* identifier = @"DDLocationCellIdentifier";
   DDChatLocationCell* cell = (DDChatLocationCell*)[tableView dequeueReusableCellWithIdentifier:identifier];
   if (!cell)
   {
      cell = [[DDChatLocationCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
   }
   cell.session =self.module.MTTSessionEntity;
   NSString* myUserID =[RuntimeStatus instance].user.objID;
   if ([message.senderId isEqualToString:myUserID])
   ...
   [cell setContent:message];
   __weak DDChatLocationCell* weakCell = cell;
   
   [cell setSendAgain:^{
      [weakCell sendLocationAgain:message];
...
//   __weak ChattingMainViewController* weakSelf = self;
   ...
   _touchDownGestureRecognizer.touchDown = ^{
      ...
      [strongSelf p_record:nil];
   };
   
   _touchDownGestureRecognizer.moveInside = ^{
      
      typeof(weakSelf) __strong strongSelf = weakSelf;
      [strongSelf p_endCancelRecord:nil];
   };

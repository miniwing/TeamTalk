...
- (void)viewDidLoad
{
   ...
   ContactsViewController *friendVc =[ContactsViewController new];
   friendVc.tabBarItem = [[UITabBarItem alloc]initWithTitle:@"通讯录" image:[UIImage imageNamed:@"contact"] selectedImage:contactSelected];
   ...
   if (item.tag ==100)
   {
      ...
      if (self.clickCount==2) {
         if ([[[RecentUsersViewController shareInstance].tableView visibleCells] count] > 0)
         {
            ...
            if(!allStop){
               [[RecentUsersViewController shareInstance].tableView scrollsToTop];
            }
         }
         self.clickCount=0;
      }

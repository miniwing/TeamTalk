...
#define TABLE_FRIEND_CONTACTS          @"friendContacts"
...
#define SQL_CREATE_ALL_CONTACTS        [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (ID text UNIQUE,Name text,Nick text,Avatar text, Department text,DepartID text, Email text,Postion text,Telphone text,Sex integer,updated real,pyname text,signature text, IsFriend integer)",TABLE_ALL_CONTACTS]

#define SQL_CREATE_FRIEND_CONTACTS     [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (ID text UNIQUE,Name text,Nick text,Avatar text, Department text,DepartID text, Email text,Postion text,Telphone text,Sex integer,updated real,pyname text,signature text, IsFriend integer)",TABLE_FRIEND_CONTACTS]
...
- (void)openCurrentUserDB {
   ...
   if (![_database open]) {
      ...
      if (dbVersion > lastDbVersion) {
         ...
         [self clearTable:TABLE_FRIEND_CONTACTS];
      ...
      [_dataBaseQueue inDatabase:^(FMDatabase *db) {
         ...
         if (![_database tableExists:TABLE_FRIEND_CONTACTS]) {
            [self createTable:SQL_CREATE_FRIEND_CONTACTS];
         }
...
- (NSArray*)messageFromSearchResult:(FMResultSet*)resultSet
{
   ...
   switch (messageType) {
      ...
      case MsgTypeMsgTypeGroupText:
         ...
         break;
      case MsgTypeMsgTypeSingleImage:
      case MsgTypeMsgTypeGroupImage:
         messageEntity.msgContentType = DDMessageTypeImage;
         break;
      case MsgTypeMsgTypeSingleLocation:
      case MsgTypeMsgTypeGroupLocation:
         messageEntity.msgContentType = DDMEssageTypeLocation;
         break;
      case MsgTypeMsgTypeSingleAudio:
      case MsgTypeMsgTypeGroupAudio:
         messageEntity.msgContentType = DDMessageTypeVoice;
         break;
      case MsgTypeMsgTypeSingleVideo:
      case MsgTypeMsgTypeGroupVideo:
         messageEntity.msgContentType = DDMEssageTypeVideo;
         break;
      case MsgTypeMsgTypeSingleUrl:
      case MsgTypeMsgTypeGroupUrl:
         messageEntity.msgContentType = DDMEssageTypeURL;
         break;
      case MsgTypeMsgTypeSingleFile:
      case MsgTypeMsgTypeGroupFile:
         messageEntity.msgContentType = DDMEssageTypeFile;
         break;
      case MsgTypeMsgTypeSingleEmotion:
      case MsgTypeMsgTypeGroupEmotion:
         messageEntity.msgContentType = DDMEssageEmotion;
         break;
      default:
         messageEntity.msgContentType = DDMessageTypeText;
         break;
...
- (MTTUserEntity*)userFromResult:(FMResultSet*)resultSet
{
   ...
   [dic safeSetObject:[NSNumber numberWithInt:[resultSet intForColumn:@"IsFriend"]] forKey:@"is_friend"];
      ...
      @try {
         [messages enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            switch (message.msgType) {
               ...
               case MsgTypeMsgTypeSingleLocation:
               case MsgTypeMsgTypeGroupLocation:
                  message.msgContentType = DDMEssageTypeLocation;
                  break;
               case MsgTypeMsgTypeSingleAudio:
               case MsgTypeMsgTypeGroupAudio:
                  debugLog(@"ddsf");
               ...
               case MsgTypeMsgTypeSingleVideo:
               case MsgTypeMsgTypeGroupVideo:
                  message.msgContentType = DDMEssageTypeVideo;
                  break;
               case MsgTypeMsgTypeSingleUrl:
               case MsgTypeMsgTypeGroupUrl:
                  message.msgContentType = DDMEssageTypeURL;
                  break;
               case MsgTypeMsgTypeSingleFile:
               case MsgTypeMsgTypeGroupFile:
                  message.msgContentType = DDMEssageTypeFile;
                  break;
               case MsgTypeMsgTypeSingleEmotion:
               case MsgTypeMsgTypeGroupEmotion:
   ...
   [_dataBaseQueue inDatabase:^(FMDatabase *db) {
      ...
      @try {
         ...
         [users enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            //ID,Name,Nick,Avatar,Role,updated,reserve1,reserve2
            BOOL result = [_database executeUpdate:sql,user.objID,user.name,user.nick,user.avatar,user.department,user.departId,user.email,user.position,user.telphone,@(user.sex),user.lastUpdateTime,user.pyname,user.signature, @(user.isFriend)];
   ...
   [_dataBaseQueue inDatabase:^(FMDatabase *db) {
      ...
      @try {
         ...
         [users enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            NSString* sql = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",TABLE_FRIEND_CONTACTS];
            //ID,Name,Nick,Avatar,Role,updated,reserve1,reserve2
            BOOL result = [_database executeUpdate:sql,user.objID,user.name,user.nick,user.avatar,user.department,user.departId,user.email,user.position,user.telphone,@(user.sex),user.lastUpdateTime,user.pyname,user.signature, @(user.isFriend)];
...
- (void)getAllFriends:(LoadAllContactsComplection )completion
{
   [_dataBaseQueue inDatabase:^(FMDatabase *db) {
      if ([_database tableExists:TABLE_FRIEND_CONTACTS])
      {
         [_database setShouldCacheStatements:YES];
         NSMutableArray* array = [[NSMutableArray alloc] init];
         NSString* sqlString = [NSString stringWithFormat:@"SELECT * FROM %@ ",TABLE_FRIEND_CONTACTS];
         FMResultSet* result = [_database executeQuery:sqlString];
         MTTUserEntity* user = nil;
         while ([result next])
         {
...
- (void)getFriendUserFromID:(NSString*)userID completion:(void(^)(MTTUserEntity *user))completion
{
   [_dataBaseQueue inDatabase:^(FMDatabase *db) {
      if ([_database tableExists:TABLE_FRIEND_CONTACTS])
      {
         ...
         NSString* sqlString = [NSString stringWithFormat:@"SELECT * FROM %@ where ID= ?",TABLE_FRIEND_CONTACTS];
         FMResultSet* result = [_database executeQuery:sqlString,userID];
         MTTUserEntity* user = nil;
         while ([result next])
         {

...
+(id)dicToUserEntity:(NSDictionary *)dic
{
    ...
    user.isFriend = [[dic safeObjectForKey:@"is_friend"] integerValue];
    return user;
...
-(id)initWithPB:(UserInfo *)pbUser
{
    ...
    if (self) {
        ...
        self.pyname = [Tool getAlpha:[Tool getChinesePinYin:pbUser.userNickName]];
        self.userStatus = pbUser.status;
        self.signature = pbUser.signInfo;
        self.isFriend = pbUser.isFriend;
    }
    return self;
}

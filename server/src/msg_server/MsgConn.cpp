...
void CMsgConn::_HandleRegistRequest(CImPdu *pPdu)
{
    ...
    TTIM_PRINTF(("msg_server::MsgConn::_HandleRegistRequest : CDBServConn : 0x%08x\n", (size_t)pDBConn));

    if (pDBConn)
    {
        CImUser *pImUser = CImUserManager::GetInstance()->GetImUserByLoginName(cMSG.user_name());
        TTIM_PRINTF(("msg_server::MsgConn::_HandleRegistRequest : GetImUserByLoginName : 0x%08x\n", (size_t)pImUser));

        if (!pImUser)
        {
            pImUser = new CImUser(cMSG.user_name());
            CImUserManager::GetInstance()->AddImUserByLoginName(cMSG.user_name(), pImUser);
        }
        pImUser->AddUnValidateMsgConn(this);

        TTIM_PRINTF(("msg_server::MsgConn::_HandleRegistRequest : CImUser : 0x%08x\n", (size_t)pImUser));
        TTIM_PRINTF(("msg_server::MsgConn::_HandleRegistRequest : AddUnValidateMsgConn : 0x%08x\n", (size_t)this));

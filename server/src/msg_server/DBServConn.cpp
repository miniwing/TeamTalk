...
void CDBServConn::_HandleRegistResponse(CImPdu *pPdu)
{
    ...
    TTIM_PRINTF(("msg_server::DBServConn::_HandleRegistResponse : result_string : %s\n", cRegistRes.result_string().c_str()));
    TTIM_PRINTF(("msg_server::DBServConn::_HandleRegistResponse : user_name : %s\n", cRegistRes.user_name().c_str()));
    ...
    TTIM_PRINTF(("msg_server::DBServConn::_HandleRegistResponse : attach_data"));

    uint32_t         nHandle    = attach_data.GetHandle();
    TTIM_PRINTF(("msg_server::DBServConn::_HandleRegistResponse : nHandle : 0x%08x\n", nHandle));

    CImUser         *pImUser    = CImUserManager::GetInstance()->GetImUserByLoginName(cRegistRes.user_name());
    TTIM_PRINTF(("msg_server::DBServConn::_HandleRegistResponse : CImUser : 0x%08x\n", (size_t)pImUser));

    CMsgConn        *pMsgConn   = pImUser->GetUnValidateMsgConn(nHandle);
    TTIM_PRINTF(("msg_server::DBServConn::_HandleRegistResponse : CMsgConn : 0x%08x\n", (size_t)pMsgConn));

    if (pMsgConn) {

        CImPdu cIMPDU;
        cIMPDU.SetPBMsg(&cRegistRes);
        cIMPDU.SetServiceId(SID_LOGIN);
        cIMPDU.SetCommandId(CID_LOGIN_RES_REGIST);
        cIMPDU.SetSeqNum(pPdu->GetSeqNum());
        pMsgConn->SendPdu(&cIMPDU);
    }
